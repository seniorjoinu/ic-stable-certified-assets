    ...
    if is_authorized {
        ...
        .await
        {
            Err((code, msg)) => trap(&format!(
                "Caller is not authorized. Failed to determine if caller is canister controller with code {code:?} and message '{msg}'"
            )),
            Ok((a,)) => {
...
async fn is_controller() -> Result<(), String> {
    ...
    {
        Err((code, msg)) => trap(&format!(
            "Failed to determine if caller is canister controller with code {code:?} and message '{msg}'"
        )),
        Ok((a,)) => {
            if a.settings.controllers.contains(&caller) {
                Ok(())
